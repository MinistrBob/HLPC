...
if [ $? -eq 0 ]; then
    ...
    ssh -T postgres@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool "
        ...
        ${PGHOME}/bin/pg_rewind -P -D ${NODE_PGDATA} --source-server=\"user=postgres host=${NEW_PRIMARY_NODE_HOST} port=${NEW_PRIMARY_NODE_PORT}\"
...
EOT
    ...
    if [ $? -ne 0 ]; then
        ...
        ssh -T postgres@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool "
            ...
            rm -rf ${NODE_PGDATA}
            ${PGHOME}/bin/pg_basebackup -h ${NEW_PRIMARY_NODE_HOST} -U $REPLUSER -p ${NEW_PRIMARY_NODE_PORT} -D ${NODE_PGDATA} -X stream -P
            cat > ${RECOVERYCONF} << EOT
primary_conninfo = 'host=${NEW_PRIMARY_NODE_HOST} port=${NEW_PRIMARY_NODE_PORT} user=${REPLUSER} application_name=${NODE_HOST} passfile=''${HOME}/.pgpass'''
recovery_target_timeline = 'latest'
primary_slot_name = '${REPL_SLOT_NAME}'
